...
namespace Fracture.Engine.Physics.Contacts
{
    ...
    /// </summary>
    public sealed class NarrowPhaseContactSolver
    {
        #region Delegates
        private delegate bool SolveDelegate(Body a, Body b);
        #endregion
        ...
        #region Fields
        private readonly SolveDelegate[][] solvers;
        ...
        #region Properties
        /// <summary>
        /// Returns the count of contacts available
        /// in the solver. These contacts are generated by
        /// the solve method.
        /// </summary>
        public int Count
        {
        ...
        public NarrowPhaseContactSolver()
        {
            ...
            // Index based mapping to solver delegates.
            solvers = new SolveDelegate[Enum.GetValues(typeof(ShapeType)).Cast<byte>().Max()][];
            
            // Circle solvers. Reduce one from indices to keep solver
            // lookup tight instead of parse array.
            solvers[(int)ShapeType.Circle - 1] = new SolveDelegate[2];

            solvers[(int)ShapeType.Circle - 1][(int)ShapeType.Circle - 1]  = SolveCircleToCircle;
            solvers[(int)ShapeType.Circle - 1][(int)ShapeType.Polygon - 1] = SolvePolygonToCircle;
            
            // Polygon solvers.
            solvers[(int)ShapeType.Polygon - 1] = new SolveDelegate[2];

            solvers[(int)ShapeType.Polygon - 1][(int)ShapeType.Circle - 1]  = SolvePolygonToCircle;
            solvers[(int)ShapeType.Polygon - 1][(int)ShapeType.Polygon - 1] = SolvePolygonToPolygon;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool DoProjectionsOverlap(in Vector2 a, in Vector2 b)
            => (a.Y >= b.X) || (a.X >= b.Y);
        ...
        private void EnqueueContact(Body a, Body b, Vector2 translation)
        {
            if (Count >= contacts.Length)
                Array.Resize(ref contacts, contacts.Length * 2);

            ref var contact = ref contacts[Count++];
        ...
        #region Solvers
        private bool SolvePolygonToPolygon(Body a, Body b)
        {
            ...
            // Get polygon shapes.
            var ash = a.Shape as PolygonShape;
            var bsh = b.Shape as PolygonShape;

            // Get points to polygons.
            var ap = ash.WorldVertices;
            var bp = bsh.WorldVertices;
            ...
            for (var i = 0; i < ax.Count; i++)
            {
                ...
                if (!DoProjectionsOverlap(in apj, in bpj))
                    return false;
                else
                {
        ...
        private bool SolvePolygonToCircle(Body a, Body b)
        {
            // Solve polygon and circle where p is polygon 
            // and c is circle.
            var p = a.Shape.Type == ShapeType.Polygon ? a : b;
            var c = p == a ? b : a;

            // Get shapes.
            var psh = (PolygonShape)p.Shape;
            var csh = (CircleShape)c.Shape;
            ...
            var pv = p.Shape.WorldVertices;
            
            // Copy axes to this span and allocate n + 1.
            var ax = new Vector2[psh.Axes.Count + 1];
            ...
            for (var i = 0; i < ax.Length; i++)
            {
                // Get projections.
                var cax = ax[i];
                var ppj = ProjectPolygon(p.Shape.WorldVertices, in cax);
                var cpj = ProjectCircle(cr, c.Shape.WorldVertices, in cax);

                if (!DoProjectionsOverlap(in ppj, in cpj))
                    return false;
                else
                {
        ...
        private bool SolveCircleToCircle(Body a, Body b)
        {
            // Get circle shapes.
            var ash = (CircleShape)a.Shape;
            var bsh = (CircleShape)b.Shape;
            
            // Distance between circles and the length.
            var distance = a.Position - b.Position;
            
            // Radius sum. 
            var rs = ash.Radius + bsh.Radius;
        ...
        /// </summary>
        public bool Solve(Body a, Body b)
        {
            ...
            // Get solver at index and solve. Reduce one from indices
            // as the lookup table is not sparse.
            return solvers[(int)a.Shape.Type - 1][(int)b.Shape.Type - 1](a, b);
        }
    }
}
